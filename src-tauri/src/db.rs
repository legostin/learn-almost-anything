...
pub struct Course {
    ...
    pub tags: Vec<String>,
    /// Fact-check source attachments (JSON: optional `url` + `image` ref). The
    /// claim text is the course `topic`; this carries the extra material.
    pub fact_input: Option<serde_json::Value>,
}
...
pub fn normalize_course_format(value: Option<&str>) -> &'static str {
    match value.map(str::trim) {
        ...
        Some("fact_check") => "fact_check",
        Some("academic_course") | _ => DEFAULT_COURSE_FORMAT,
    }
}

const COURSE_COLS: &str = "id, topic, title, language, course_format, status, agent, created_at, updated_at, catalog_origin_id, catalog_version, catalog_synced_at, space_id, strict_sources, translated_from, category, generation_profile, learner_profile, roadmap_id, roadmap_skill, mcp_servers, catalog_server_url, tags, fact_input";
...
/// Persist the fact-check source attachments as a JSON string (or clear it).
pub fn set_fact_input(
    conn: &Connection,
    id: &str,
    fact_json: Option<&str>,
) -> Result<(), rusqlite::Error> {
    conn.execute(
        "UPDATE courses SET fact_input = ?2 WHERE id = ?1",
        rusqlite::params![id, fact_json],
    )?;
    Ok(())
}
