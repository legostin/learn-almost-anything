...
// Style-review editorial pass: check the article against the selected writing
// style and revise drift, without harming accuracy or formatting. Returns the
// (possibly revised) article plus a short notes line recording any unresolved
// style/accuracy trade-offs.
export async function submoduleStyleReview(params, ctx) {
  ...
  // No style guidance => nothing to enforce; return the article untouched.
  if (!guidance) {
...
2. Preserve factual accuracy. If the style and a NECESSARY technical term
   conflict (e.g. a required term in a plain-language style), keep the term and
   note the trade-off — never sacrifice correctness for tone.
...
{"article":"<full revised article markdown>","notes":"<1-3 sentences on what you changed for style, plus any UNRESOLVED style/accuracy trade-off; empty string if nothing changed>"}`;
...
export async function generateTest({ topic, language, courseFormat, submodulePath, article, modelConfig, category, genProfile, structure, learnerProfile, style }, ctx) {
...
export async function generateFlashcards(
  { topic, language, courseFormat, submodulePath, article, modelConfig, category, genProfile, learnerProfile, style },
  ctx
) {
  if (typeof article !== "string" || !article.trim()) {
 ...
 */
export async function generateAssignments({ topic, language, courseFormat, submodulePath, article, modelConfig, category, genProfile, learnerProfile, style }, ctx) {
