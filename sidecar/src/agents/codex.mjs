...
function structureBounds(courseFormat) {
  switch (normalizeCourseFormat(courseFormat)) {
    ...
    case "fact_check":
      return { modMin: 1, modMax: 1, subMin: 1, subMax: 1 };
    case "podcast_series":
...
function factCheckInputBlock(courseFormat, factInput) {
  ...
  if (typeof fi.url === "string" && fi.url.trim()) {
    lines.push(
      `- Source link: ${fi.url.trim()}\n  Fetch and read this page and base your verdict on what it actually says.`
    );
  }
  if (typeof fi.imagePath === "string" && fi.imagePath.trim()) {
    lines.push(
      `- An image is attached as the fact to check. Examine the attached image and verify the claim it makes or that is made about it.`
    );
  }
...
// When an image path is supplied (a fact-check with an attached image), send the
// prompt as input items with a local_image so Codex can examine it via vision.
function withImageInput(prompt, opts) {
  const imagePath = opts?.imagePath;
  if (typeof imagePath === "string" && imagePath.trim()) {
    return [
      { type: "text", text: prompt },
      { type: "local_image", path: imagePath.trim() },
    ];
  }
  return prompt;
}
...
async function draftArticleInternal(
  ...
  const pedagogyBlock =
    categoryPedagogyBlock(category, lang, genProfile?.pedagogyIntensity || "standard") +
    learnerProfileBlock(learnerProfile);
  const memoryBlock =
...
${languageStyleGuide(lang)}`;
  ...
  const text = await runStreamed(prompt, draftSchema, onProgress, {
    ...
    customMcp,
    imagePath:
      normalizeCourseFormat(courseFormat) === "fact_check" &&
      factInput &&
      typeof factInput.imagePath === "string"
        ? factInput.imagePath
        : undefined,
  });
  const parsed = JSON.parse(text);
...
const classifySchema = {
  type: "object",
  additionalProperties: false,
  required: ["decision", "video_heavy"],
  properties: {
    decision: { type: "string", enum: ["ok", "caution", "refuse"] },
    reason: { type: "string" },
...
Separately, set "video_heavy": true when the subject is learned mainly by WATCHING and is easier to find on YouTube — internet memes and viral moments, dances or sports moves, music / instrument technique, cooking, crafts / DIY, makeup, software UI walkthroughs. This is an INDEPENDENT, positive content hint, unrelated to the safety decision: set it whenever it fits, including for an "ok" or "caution" topic. It never restricts anything.

Return the decision, an optional reason, and video_heavy.`;
  ctx?.progress?.({ label: "checking topic" });
  const text = await runStreamed(prompt, classifySchema, ctx?.progress, { modelConfig });
  let parsed;
  try {
    parsed = JSON.parse(text);
  } catch {
